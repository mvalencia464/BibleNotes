...
export const saveBibleBook = async (versionId, versionName, bookName, data) => {
...
export const getBibleBook = async (versionId, bookName) => {
    const db = await initDB();
    const key = `${versionId}-${bookName}`;
...
export const deleteBibleVersion = async (versionId) => {
    const db = await initDB();
    const allKeys = await db.getAllKeys(STORE_NAME);
...
export const saveSermons = async (data) => {
    const db = await initDB();
...
export const areSermonsDownloaded = async () => {
